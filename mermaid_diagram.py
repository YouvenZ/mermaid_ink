...
class MermaidGenerator(inkex.EffectExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument(
            "--mermaid_file", type=str, default="", help="Mermaid file path"
        )
        pars.add_argument(
            "--use_file", type=inkex.Boolean, default=False, help="Use external file"
        )

        # Style parameters
        pars.add_argument("--theme", type=str, default="default", help="Mermaid theme")
        pars.add_argument(
            "--background", type=str, default="white", help="Background color"
        )
        ...
        # Format parameters
        pars.add_argument(
            "--output_format", type=str, default="svg", help="Output format"
        )
        pars.add_argument(
            "--scale_factor", type=float, default=1.0, help="Scale factor"
        )
        pars.add_argument("--quality", type=int, default=2, help="Quality level")
        pars.add_argument(
            "--embed_image", type=inkex.Boolean, default=True, help="Embed image"
        )

        # Enhanced positioning parameters
        pars.add_argument(
            "--position_mode", type=str, default="center", help="Position mode"
        )
        ...
        pars.add_argument(
            ...
            type=inkex.Boolean,
            default=False,
            help="Position relative to selection",
        )
        pars.add_argument(
            "--align_to_page",
            type=inkex.Boolean,
            default=True,
            help="Align to page or viewport",
        )

        # Enhanced scaling parameters
        pars.add_argument(
            "--auto_scale", type=inkex.Boolean, default=False, help="Auto-scale to fit"
        )
        pars.add_argument(
            "--max_width", type=int, default=0, help="Maximum width (0=no limit)"
        )
        pars.add_argument(
            "--max_height", type=int, default=0, help="Maximum height (0=no limit)"
        )
        pars.add_argument(
        ...
        pars.add_argument(
            ...
            type=inkex.Boolean,
            default=False,
            help="Lock object after insertion",
        )

        # Config parameters
        pars.add_argument(
            "--mermaid_cli_path", type=str, default="mmdc", help="Mermaid CLI path"
        )
        pars.add_argument(
            "--use_puppeteer", type=inkex.Boolean, default=True, help="Use Puppeteer"
        )
        ...
        pars.add_argument(
            "--viewport_width", type=int, default=1920, help="Viewport width"
        )
        pars.add_argument(
            "--viewport_height", type=int, default=1080, help="Viewport height"
        )
        pars.add_argument(
            "--keep_temp_files",
            type=inkex.Boolean,
        ...
        pars.add_argument(
            "--quiet_mode", type=inkex.Boolean, default=True, help="Quiet mode"
        )
        ...
        pars.add_argument(
            ...
            type=str,
            default="inkscape",
            help="Inkscape executable path",
        )
        pars.add_argument(
            "--pdf_poppler",
            type=inkex.Boolean,
            default=True,
            help="Use PDF poppler for import",
        )
        pars.add_argument(
            "--fit_to_content",
            type=inkex.Boolean,
            default=True,
            help="Fit to content (no empty space)",
        )
        ...
        pars.add_argument(
            ...
            type=inkex.Boolean,
            default=False,
            help="Create new layer for diagram",
        )
        pars.add_argument(
            "--layer_name", type=str, default="Mermaid Diagrams", help="Layer name"
        )

        # New: Object properties
        pars.add_argument("--object_id", type=str, default="", help="Custom object ID")
        pars.add_argument(
            "--add_title", type=inkex.Boolean, default=False, help="Add title element"
        )
        pars.add_argument(
            "--add_desc",
            type=inkex.Boolean,
            default=False,
            help="Add description element",
        )
    ...
    def effect(self):
        ...
        try:
            ...
            if not self.check_mermaid_cli():
                inkex.errormsg(
                    f"Mermaid CLI not found at: {self.options.mermaid_cli_path}\n\n"
    ...
    def generate_diagram_pdf(self, mermaid_code):
        ...
        # Build command - generate PDF; -f fits the PDF page to the diagram bounds.
        cmd = [self.options.mermaid_cli_path, "-f", "-i", input_file, "-o", pdf_file]
    ...
    def import_pdf_as_svg(self, pdf_file):
        ...
        try:
            ...
            x, y = self.calculate_position(
                actual_width * final_scale, actual_height * final_scale
            )
            ...
            if self.options.add_desc:
                desc = inkex.Desc()
                desc.text = (
                    self.diagram_code[:200] + "..."
                    if len(self.diagram_code) > 200
                    else self.diagram_code
                )
                group.append(desc)
    ...
    def convert_pdf_to_png(self, pdf_file):
        ...
        try:
            ...
            base_dpi = (
                72 * self.options.quality
            )  # quality 1=72dpi, 2=144dpi, 3=216dpi, 4=288dpi
